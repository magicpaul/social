...
class UserFriendshipDecoratorTest < Draper::TestCase
	context "sub_message" do
		context "with a pending user friendship" do
			...
			should "return pending message" do
				assert_equal "Do you really want to be friends with Jim Last?", @decorator.sub_message
			end
		end
		context "with a accepted user friendship" do
			...
			should "return accepted message" do
				assert_equal "You are friends with Jim Last.", @decorator.sub_message
			end
		end
	end
